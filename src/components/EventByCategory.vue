  ...
  <v-overlay
     ...
     >
       ...
       >
         ...
         <country-select v-model="country" :country="country" countryName="true" placeholder="Country" class="loc"/>
         <region-select v-model="region" :country="country" countryName="true" regionName="true" v-on:input="locat()"  :region="region" placeholder="City" class="loc" />
   ...
   <v-container fluid class="ma-0 pt-0 pb-14 px-0">
     <v-row class="#101828 mt-0 px-1" style="background-color:#101828;">
         ...
         <v-col cols="6" class="pr-1 py-0 pb-0">
             <router-link to="/TodayHome"> <v-btn class="view">View Venues</v-btn></router-link>
         </v-col>
       </v-row>
     ...
     <v-row class="px-3 py-0" style="background-color: #1d2939">
       <v-col cols="6" v-for="item in events"
         ...
         <div class="colum">
           <div class="gallery">
             <img width="100%" height="90px" crossorigin="anonymous" :src="`http://138.68.27.231:3000/projectdata/eventspic/${item.eventimage}`" />
           </div>
 ...
 export default {
   ...
   methods: {
     ...
     async loga() {
         ...
         try {
           let result = await axios.get(`http://138.68.27.231:3000/api/v1/events/getallbycity?search=${this.region},${this.country}`);
     ...
     close() {
       ...
       console.log("All events",result.data.data.events);
       this.events=result.data.data.events
